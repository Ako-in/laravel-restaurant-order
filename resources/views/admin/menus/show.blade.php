    ...
    <div class="col-md-8"> {{-- 中画面以上で8列分の幅を取り、中央寄せ --}}
        ...
        <div class="text-center mb-4"> {{-- 画像を中央寄せし、下部にマージン --}}
            @if ($menu->image_file) {{-- $menu->image_file が存在するかをチェック --}}
                <img src="{{ asset('storage/' . $menu->image_file) }}" alt="Menu Image" class="img-fluid rounded" style="max-width: 300px; height: auto;">
            @else
                <img src="{{ asset('storage/images/noimage.png') }}" alt="No Image" class="img-fluid rounded" style="max-width: 300px; height: auto;">
            @endif
        </div>
