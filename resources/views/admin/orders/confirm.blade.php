...
<div>
    ...
    <table>
        <tr style="text-align: center;">
            ...
            <th style="padding: 10px;">注文数量</th>
            <th style="padding: 10px;">単価</th>
            <th style="padding: 10px;">小計</th>
            {{-- <th>テーブル番号</th> --}}
            <th style="padding: 10px;">在庫</th>
            {{-- <th>注文変更前</th>{{-- データを取得のみ --}}
            <th style="padding: 10px;">数量変更後</th>{{-- formタグで数量変更admin.orders.updateQtyへ飛ばす --}}
            {{-- <th>詳細</th> --}}
            <th style="padding: 10px;">個別ステータス</th>
        </tr>
        ...
        @foreach ($order->orderItems as $item)
            <tr style="text-align: center;">
                ...
                {{-- <td>{{$order->id}}</td>注文ID --}}
                <td style="padding: 10px;">{{ $item->menu_id ?? '不明' }}</td>{{-- メニューID --}}
                <td style="padding: 10px;">{{ $item->menu_name ?? '不明' }}</td>{{-- メニュー名 --}}
                <td style="padding: 10px;">{{ $item->qty }}</td>{{-- 数量変更前 --}}
                <td style="padding: 10px;">{{ number_format($item->price) }}円</td>{{-- 単価 --}}
                {{-- <td>{{ number_format($item->subtotal) }}円</td> 小計税抜 --}}
                {{-- <td>{{ number_format($item->subtotalTax) }}円</td> 小計税込 --}}
                <td style="padding: 10px;">{{ number_format($item->price * $item->qty) }}</td>{{-- 小計 --}}
                {{-- <td>{{ number_format($order->subtotal) }}円</td> --}}
                {{-- <td>{{ number_format($order->orderItems->sum(function($item) {
            return $item->price * $item->qty;
        })) }}円</td> --}}
                {{-- <td>{{$order->table_number}}</td> --}}
                <td style="padding: 10px;">{{ $item->menu ? $item->menu->stock : '不明' }}</td>{{-- 在庫 --}}
                {{-- <td><a href="{{ route('admin.orders.show', $order->id) }}">詳細</a></td> --}}
                <td style="padding: 10px;">
                    <form action="{{ route('admin.orders.updateQty', ['item' => $item->id]) }}"method="POST"
                        class="d-flex align-items-center">
                        @csrf
                        @method('PUT')
                        <div class="input-group">
                            <input type="number" name="qty" value="{{ $item->qty }}" min="0"
                                class="form-control form-control-sm me-2" style="width: 80px;" {{-- 注文全体のステータスが完了・キャンセルの場合は個別の数量変更も不可にする --}}
                                @if ($order->status === 'completed' || $order->status === 'canceled') disabled @endif>
                            <button type="submit" class="btn btn-sm btn-info"
                                @if ($item->status === 'completed' || $item->status === 'canceled') disabled @endif>数量更新
                            </button>
                        </div>
                ...
                <td style="padding: 10px;">
                    {{-- アイテム個別のステータス変更 --}}
                    <form action="{{ route('admin.orders.updateOrderItemStatus', ['item' => $item->id]) }}" method="POST"
                        class="d-flex align-items-center">
                        @csrf
                        @method('PUT')
                        ...
                        <button type="submit" class="btn btn-sm btn-primary"style="width: 100px;"
                            @if ($order->status === 'completed' || $order->status === 'canceled') disabled @endif>更新
                        </button>
                </td>
        ...
        <div class="">
            ...
            {{-- 戻るボタン --}}
            <a href="{{ route('admin.orders.index') }}" class="btn btn-secondary mb-3">一覧へ戻る</a>
